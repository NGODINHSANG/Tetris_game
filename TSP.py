...
def get_random_style():
    ...
    block = [i for i in sum(
        [[[j, i] if blockstyle[0][i][j] == 1 else None for j in range(len(blockstyle[0][i]))] for i in
         range(len(blockstyle[0]))], []) if i] + [blockstyle[1]]
